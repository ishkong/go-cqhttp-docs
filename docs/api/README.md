...
| 说明                                                                                                          |
|-------------------------------------------------------------------------------------------------------------|
| 这个其实说的是 websocket 建立连接时的 URL, 你可以连接 `/` 终结点, 也可以连接 `/api` 终结点或者 `/event` 终结点. 它们的区别在于, `/api`只会响应`api`调用并上报调用结果, `/event`只会上报事件而不响应`api`调用    |
| 一个 JSON 数据, 其中包含了请求 API 的终结点, 以及其API请求的参数                                                                          |
| 在调用 api 时, 你还可以传入 `echo` 字段, 然后响应数据中也会有一个值相同的 `echo` 字段, 可以使用这个方式来甄别 "这个响应是哪个请求发出的", 你可以为每一个请求都使用一个唯一标识符来甄别, 例如`Guid`或者自增序列 |
